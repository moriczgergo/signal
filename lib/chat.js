...
module.exports = function chatRouter(share) {
    ...
    var models = require('./mongoose')(share.mongoose);
    ...
    var authCheckInterval = setInterval(function authCheck() { // check if logged-in chat tokens still exist or not
        ...
        chatClients.filter(x => x.auth).forEach(client => { // auth check code
            ...
            models.Token.findOne({ token: client.auth }).then(doc => {
    ...
    router.ws('/', function onWSConnect(ws, req) {
        ...
        ws.on('message', function onWSMessage(msg) {
            ...
            if (o.type == "hello") {
                ...
                models.Token.findOne({ token: o.token }).then(doc => {
                ...
                models.Token.findOne({ token: chatClients.find(x => id == x.id ).auth }).then(doc => {
                    ...
                    models.Account.findById(doc.account).then(accdoc => {
                        ...
                        if (o.private) {
                            models.Account.findOne({ users: o.channel }).then(tardoc => {
                                if (!tardoc) {
